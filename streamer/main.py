...
DEFAULT_RTC_CONFIG = """{
  ...
  "iceServers": [
    {
      "urls": [
        "stun:zefron5.cerit-sc.cz:3478"
      ]
    },
    {
      "urls": [
        "turn:zefron5.cerit-sc.cz:3478?transport=tcp",
        "turn:zefron5.cerit-sc.cz:3478?transport=udp"
       ],
       "username": "",
       "credential": ""
    }
...
def make_turn_rtc_config_json(host, port, username, password):
  ...
  "blockStatus": "NOT_BLOCKED",
  "iceTransportPolicy": "all"
}""" % (host, port, host, port, host, port, username, password)
...
def fetch_coturn(uri, user, auth_header_name):
    ...
    conn.request("GET", "%s?username=%s" % (parsed_uri.path, user))
...
def main():
    ...
    def set_fps_handler(fps):
        ...
        if fps != curr_fps:
            if not app.set_video_framerate(float(fps)):
